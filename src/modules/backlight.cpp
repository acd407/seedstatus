...
const std::string BacklightModule::MAX_BRIGHTNESS_PATH = "/sys/class/backlight/amdgpu_bl1/max_brightness";
...
void BacklightModule::init() {
    ...
    if (watch_fd_ == -1) {
        std::cerr << "Failed to add watch for " << BRIGHTNESS_PATH << ": " << strerror(errno) << std::endl;
        close(inotify_fd_);
        inotify_fd_ = -1;
        setOutput("󰛨", Color::DEACTIVE);
        setInterval(1); // 设置重试间隔
