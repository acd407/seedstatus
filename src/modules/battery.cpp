...
BatteryModule::BatteryModule() : Module("battery"), dbus_conn_(nullptr), detailed_mode_(false) {
...
void BatteryModule::init() {
    ...
    if (dbus_error_is_set(&err)) {
        std::cerr << "Failed to get DBus connection: " << err.message << std::endl;
        dbus_error_free(&err);
    ...
    // 添加D-Bus信号匹配规则
    std::string match_rule = "type='signal',interface='org.freedesktop.DBus.Properties',"
                           "path='" + BATTERY_PATH + "',arg0='" + DEVICE_INTERFACE + "'";
    dbus_bus_add_match(dbus_conn_, match_rule.c_str(), &err);
    if (dbus_error_is_set(&err)) {
        std::cerr << "Failed to add DBus match rule: " << err.message << std::endl;
        dbus_error_free(&err);
        dbus_connection_unref(dbus_conn_);
        dbus_conn_ = nullptr;
        setOutput("󱠵", Color::DEACTIVE);
...
void BatteryModule::update() {
    try {
        ...
        // 格式化输出
        std::string output = formatOutput(state, static_cast<uint64_t>(percentage), 
                                         energy, energy_rate, time);
...
void BatteryModule::dbusGetProperty(const std::string& device_path, 
    ...
    DBusMessage* reply = dbus_connection_send_with_reply_and_block(dbus_conn_, msg, -1, &err);
    dbus_message_unref(msg);
...
void BatteryModule::getBatteryInfo(BatteryState& state, double& percentage, 
                                  int64_t& time, double& energy, double& energy_rate) {
    if (!dbus_conn_) {
...
std::string BatteryModule::formatOutput(BatteryState state, uint64_t percentage, 
